...
local function should_yield(text, option)
    ...
    local char = unicode_chars(text)[1]
    if not is_cjk(char) then
        return true
    end

    if _G._092core_db:lookup(char) ~= "" then
        return true
    end
    return false
end
...
local function init(env)
    ...
    _G._092core_db = ReverseDb("build/092core.reverse.bin")
end
